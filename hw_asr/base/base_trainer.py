...
class BaseTrainer:
    ...
    def __init__(self, model: BaseModel, criterion, metrics, optimizer, scheduler, config, device):
        ...
        if config.pretrained is not None:
            self._from_pretrained(config.pretrained)
    ...
    def train(self):
        try:
            ...
            ckpt_path = self._save_checkpoint(self._last_epoch, save_best=False)
            self.writer.add_ckpt(f'{self.config["name"]}_ckpt', ckpt_path)
            raise e
    ...
    def _from_pretrained(self, pretrained_path):
        ...
        :param pretrained_path: Checkpoint path for pretrained model
        """
        pretrained_path = str(pretrained_path)
        self.logger.info("Loading pretrained model: {} ...".format(pretrained_path))
